...
public final class Util {
  ...
  public static void checkLinkage(int program) {
    int linked = glGetProgram(program, GL20.GL_LINK_STATUS);
    if (linked == GL11.GL_FALSE) {
      int logLength = glGetProgram(program, GL20.GL_INFO_LOG_LENGTH);
  ...
  public static void checkCompilation(int shader) {
    int compiled = glGetShader(shader, GL20.GL_COMPILE_STATUS);
    if (compiled == GL11.GL_FALSE) {
      int logLength = glGetShader(shader, GL20.GL_INFO_LOG_LENGTH);
